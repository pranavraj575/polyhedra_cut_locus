...
STARTS = [
    [["-" if j >= i else exp_simplify(item[0][dim]) for (j, item) in enumerate(row)] for (i, row) in enumerate(L)]
    for dim in range(2)
]
VECTORS = [
    [["-" if j >= i else exp_simplify(item[1][dim]) for (j, item) in enumerate(row)] for (i, row) in enumerate(L)]
    for dim in range(2)
]
