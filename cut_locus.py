...
display_group.add_argument("--center-pt", action="store_true", required=False, help="add center point to faces")
...
display_group.add_argument("--leg-font-size", type=int, required=False, default=None, help="legend font size")
PARSER.add_argument(
    "--ignore-points",
    action="store_true",
    required=False,
...
def get_marks(args):
    ...
    for mp in args.mark:
        ...
        marks.append((mp[0], (float(mp[1]), float(mp[2])), mp[3] if len(mp) >= 4 else None, mp[4] if len(mp) == 5 else None))
    return marks
