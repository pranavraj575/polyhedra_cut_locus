...
PARSER.add_argument(
    "--dir", action="store", required=False, help="root of file tree to search (defaults to parent dir of main tex file)"
)
PARSER.add_argument(
    "--output-file", action="store", required=False, help="where to save new tex file (defaults to /output/<main tex file name>)"
)
PARSER.add_argument("--comments-remove", action="store_true", required=False, help="delete anything line beginning with '%%'")
PARSER.add_argument(
    "--cmds-remove",
    action="store",
    nargs="+",
...
output_file = args.output_file if args.output_file else os.path.join("output", os.path.basename(main_file))
