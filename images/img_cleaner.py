...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        "--replacement", type=int, nargs="+", required=False, default=None, help="pixel to set border as (defaults to --color)"
    )
    PARSER.add_argument("--border-size", type=int, default=1, required=False, help="whitespace to add to/keep on border")
    PARSER.add_argument("--img-file", nargs="*", default=[], required=False, help="file(s) to remove whitespace border of")
    ...
    for thing in img_files:
        ...
        if border_size > 0:
            out_img = (
                np.ones((img.shape[0] + border_size * 2, img.shape[1] + border_size * 2, img.shape[2]), dtype=img.dtype) * 255
            )
