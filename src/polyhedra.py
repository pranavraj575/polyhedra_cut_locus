...
class ConvexPolyhderon(Shape):
    ...
    def plot_voronoi_star_unfolding(
        self,
        p,
        source_fn,
        ax=None,
        ...
        for sink_fn in self.faces:
            if sink_fn != source_fn:
                voronoi_diagram = self.get_voronoi_diagram(
                    p=p,
                    source_fn=source_fn,
                    sink_fn=sink_fn,
                    diameter=diameter,
                ...
                if voronoi_diagram is not None:
                    ...
                    point_pair_to_segment = {
                        point_pair: (a.reshape((2, 1)), b.reshape((2, 1)))
        ...
        for sink_fn in self.faces:
            if sink_fn != source_fn:
                ...
                if voronoi_diagram is not None:
                    (point_pair_to_segment, (relevant_points, relevant_bound_paths, relevant_cells)) = voronoi_diagram
                    ...
                    for path in relevant_bound_paths:
                        ...
                        if path is not None:
                            ...
                            for bnd, F in path[::-1]:  # start with sink face
                                ...
                                vertex_cycles.append(np.concatenate([F.vertices[i % num_v][0] for i in range(1 + num_v)], axis=1))
                                ...
                                segments = {
                                    point_pair: (bnd.shift_point(a), bnd.shift_point(b))
    ...
    def _plot_label_face(
        ...
        ax.annotate(
            str(name) + orient_string, (center[0], center[1]), rotation=np.degrees(theta), color=color, zorder=label_zorder
        )

    def plot_unfolding_from_sink(
        self,
        p,
        source_fn,
        sink_fn,
        ...
        for pt_idx, (pt, path) in enumerate(zip(relevant_points, relevant_bound_paths)):
            ...
            if path is not None:
                ...
                if (i_to_display is not None) and (disp_i != i_to_display):
                    ...
                    continue
                face_tracking = [
                    [v.copy() for (v, _) in source.get_vertices()]
                ]  # tracking the vertices of each face and their eventual location
        ...
        ax.set_ylim(ylim)
        ax.legend()
        return all_trans_shown, i_to_display is None
            ...
            for j in range(m):
                ...
                if face is not None:
                    ...
                    for (p1, p2), (bound, f) in path:
                        ...
                        ploot(i, j).plot([x, xp], [y, yp], label=label, alpha=0.5)
    ...
    def interactive_vornoi_plot(
        ...
        if event_key is not None:
            fig.canvas.mpl_connect(event_key, mouse_event)
            self.plot_face_boundaries(axs, legend=legend, legend_kwargs=legend_kwargs)
        else:
            fn, p = source_fn_p
    ...
    def interactive_unfold(
        ...
        if source_fn_p is not None:
            ...
            if not self.faces[self.extra_data["unwrap_source_fn"]].within_bounds(self.extra_data["p"]):
                ...
                print(
                    "WARNING: point " + temp + " not in face, taking closest point: " + str(tuple(self.extra_data["p"].flatten()))
                )
        ...
        def spin():
            ...
            if (self.extra_data["unwrap_source_fn"] is not None and self.extra_data["unwrap_sink_fn"] is not None) or (
                ...
                if voronoi_star:
                    self.plot_voronoi_star_unfolding(
                        p=self.extra_data["p"],
                    ...
                    for i, (zero, xvec, yvec, p) in enumerate(all_trans_shown):
                        ...
                        shiftt = np.array(
                            [
                                int(np.sign(val)) * sym.sqrt(round(val**2)) if abs(round(val**2) - val**2) <= 1e9 else val
                                for val in zero.flatten()
                            ]
                        )
                        print("\tshift:", shiftt)
                        rot_frac = fractions.Fraction(
                            np.arctan2((xvec - zero)[1], (xvec - zero)[0])[0] / np.pi
                    ...
                    if single_display and (all_trans_shown is not None):
                        if save is not None:
                            if not done_plotting:
                                plt.savefig(
                                    os.path.join(
                                        os.path.dirname(save), "point_" + str(i_to_display) + "_" + os.path.basename(save)
                                    ),
                                    **save_kwargs,
                                )
                            if done_plotting:
        ...
        def clicked_mouse(event):
            ...
            if self.extra_data["unwrap_source_fn"] is None:
                ...
                self.extra_data["unwrap_sink_fn"] is None
                and fc is not None
                and fc.name is not self.extra_data["unwrap_source_fn"]
            ):
        ...
        fig.canvas.mpl_connect("button_press_event", clicked_mouse)
        if track:
            fig.canvas.mpl_connect("motion_notify_event", moved_mouse)
        self.plot_face_boundaries(axs, legend=legend)
        plt.suptitle("Click $p$")
        done_plottin = spin()
