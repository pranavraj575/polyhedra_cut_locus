...
class Bound:
    def __init__(self, m, b, s, T, si, dimension=None, name=None, identifier=""):
        ...
        if name is None:
            base_id = (
                str(tuple(self.m.flatten()))
                + str(self.b)
                + str(tuple(self.s.flatten()))
                + str(tuple(self.T.flatten()))
                + str(tuple(self.si.flatten()))
                + str(self.dimension)
            )
    ...
    def check_valid(self, dimension):
        ...
        if np.shape(self.m)[0] != 1 or np.shape(self.s)[1] != 1 or np.shape(self.si)[1] != 1:
            raise Exception("tried putting in bounds of incorrect dimensions")
        if (
            np.shape(self.T)[0] != np.shape(self.T)[1]
            or np.shape(self.m)[1] != np.shape(self.s)[0]
            or np.shape(self.s)[0] != np.shape(self.si)[0]
            or np.shape(self.si)[0] != np.shape(self.m)[1]
            or np.shape(self.m)[1] != np.shape(self.T)[0]
        ):
    ...
    def concatenate_with(self, T=None, s=None):
        ...
        """
        if T is None:
            T = np.identity(self.dimension)
        if s is None:
            s = np.zeros((self.dimension, 1))
