from utils.shape_argparser import (
    figsize_from_args,
    check_face_name,
    get_source_fn_p_from_args,
    shape_from_args,
    parse_args,
    get_parser_and_display_group,
)

PARSER, display_group = get_parser_and_display_group()
...
PARSER.add_argument(
    "--voronoi-star", action="store_true", required=False, help="unfold from source face to create a voronoi star"
)
PARSER.add_argument(
    "--ignore-points",
    action="store_true",
...
display_group.add_argument("--single-display", action="store_true", required=False, help="display only one path at a time")
...
display_group.add_argument("--label-dist-line", type=float, required=False, default=0.3, help="distance to label each line from")
display_group.add_argument(
    "--point-names", action="store", nargs="*", required=False, default=None, help="names of each point", metavar="p0 p1"
)
display_group.add_argument(
    "--mark", action="append", nargs="*", required=False, help="mark point on cut locus faces", metavar="X Y <COLOR>, <LABEL>"
)
...
args = parse_args(PARSER, display_group)
