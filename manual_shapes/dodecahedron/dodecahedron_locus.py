...
if __name__ == "__main__":
    ...
    p = np.array([[0.0], [0.0]])
    p = np.array([[0.0], [0.2]])
    p = 0.2 * coltation(np.pi / 2 - np.pi * 2 / 5)
    p = 0.2 * coltation(np.pi / 2 - np.pi * 2 / 5) + 0.075 * coltation(np.pi / 2)
    ...
    cube.plot_faces(
        save_image=os.path.join(folder, name), show=False, figsize=(15, 9), voronoi=(p, fn, None), greedy_computation=False
    )
