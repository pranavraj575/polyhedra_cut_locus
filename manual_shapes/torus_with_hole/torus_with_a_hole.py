...
def plot_bounds(nine=False):
    ...
    plt.plot((0.5 - length / 2, 0.5 + length / 2), (1 - ht, 1), "-", color=color, linewidth=linewidth, alpha=alpha)
    plt.plot((0.5 - length / 2, 0.5 + length / 2), (1 + ht, 1), "-", color=color, linewidth=linewidth, alpha=alpha)

    plt.plot((0.5 - length / 2, 0.5 + length / 2), (ht, 0), "-", color=color, linewidth=linewidth, alpha=alpha)
    plt.plot((0.5 - length / 2, 0.5 + length / 2), (-ht, 0), "-", color=color, linewidth=linewidth, alpha=alpha)

    offset = length / 2

    plt.plot(
        (1 - ht, 1), (0.5 - length / 2 + offset, 0.5 + length / 2 + offset), "-", color=color, linewidth=linewidth, alpha=alpha
    )
    plt.plot(
        (1 + ht, 1), (0.5 - length / 2 + offset, 0.5 + length / 2 + offset), "-", color=color, linewidth=linewidth, alpha=alpha
    )
    ...
    plt.plot((-ht, 0), (0.5 - length / 2 + offset, 0.5 + length / 2 + offset), "-", color=color, linewidth=linewidth, alpha=alpha)
    plt.plot((+ht, 0), (0.5 - length / 2 + offset, 0.5 + length / 2 + offset), "-", color=color, linewidth=linewidth, alpha=alpha)
...
def geodesic_counts_complicated(points, consider, consider_obs, obs_r, tol=TOL):
    ...
    alphas = np.arccos(np.matmul(np.expand_dims(to_pt, -2), np.expand_dims(to_cons, -1)).reshape(k) / (to_pt_dist * to_cons_dist))
...
def which_to_reconsider(points, consider, consider_obs, obs_r):
    ...
    stuff = np.logical_and(
        which_intersect_obs(points, consider, consider_obs, obs_r), which_shortest_paths(points, consider, tol=TOL)
    )
...
def which_paths_intersect_which_obs(a, b, consider_obs, obs_r):
    ...
    blast = np.matmul(
        np.expand_dims(pt_to_obs / np.expand_dims(np.linalg.norm(pt_to_obs, axis=-1), -1), -2), np.expand_dims(a - b, -1)
    )
...
vv = vv + (1 / (nx + ny)) * (np.random.random(vv.shape) - 0.5)
