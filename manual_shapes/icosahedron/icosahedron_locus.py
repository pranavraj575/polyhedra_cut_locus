...
if __name__ == "__main__":
    ...
    p = np.array([[0.0], [0.0]])
    p = np.array([[0.0], [0.2]])
    p = 0.1 * coltation(-np.pi / 6)
    p = 0.3 * coltation(-np.pi / 6)
    p = np.array([[0.0], [0.2]])
    p = np.array([[0.0], [0.2]]) + 0.125 * coltation(-np.pi / 6)
    p = np.array([[0.0], [0.2]]) - 0.125 * coltation(-np.pi / 6)
    ...
    cube.plot_faces(
        save_image=os.path.join(folder, name), show=False, figsize=(15, 12), voronoi=(p, fn, None), greedy_computation=False
    )
